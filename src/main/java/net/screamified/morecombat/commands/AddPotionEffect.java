...
public class AddPotionEffect implements TabExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("addpotioneffect")) {
            if (args.length >= 3) {
                PotionEffectHandler.AddPotionEffect(((Player) sender).getPlayer(), getHeldItem, args);
                return true;
            } else {
                player.sendMessage(ChatColor.RED + "[/addpotioneffect <Potion Effect> <Amplify> <Duration eg: M:S, MM:SS>]");
                return true;
            }
        }
